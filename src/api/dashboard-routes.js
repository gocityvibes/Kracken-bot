// src/api/dashboard-routes.js (clean live-only router)
...
// Attempt to read a CSV file if present
...
app.get('/api/summary', (_req, res) => {
  return safeJson(res, () => {
    ...
    return res.json({ ok:true, count, wins, losses, zeros, winRatePct, netPNL: +netPNL.toFixed(2),
                      grossWin:+grossWin.toFixed(2), grossLoss:+grossLoss.toFixed(2),
                      profitFactor, avgWin, avgLoss, bySide, bySymbol, byExitReason, byStopLevel });
  });
});

// CSV downloads (if the files exist in working dir)
app.get('/api/download/trades.csv', (_req, res) => {
  ...
  res.setHeader('Content-Type','text/csv'); return res.send(txt);
});
...
// No backtest in live-only build
app.all('/api/backtest*', (_req, res) =>
  res.status(410).json({ ok:false, error:'Backtest removed in live-only build' })
);
